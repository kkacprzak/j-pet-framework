...
#ifndef SCOPE_LOADER_MODULE_H
#define SCOPE_LOADER_MODULE_H
 ...
 * Reads oscilloscope ASCII data and produces JPetShapedSignal structures.
...
public:
  ...
  static std::map<int, std::map<std::string, int>> groupScopeFileNamesByTimeWindowIndex(const std::map<std::string, int>& scopeFileNames);
  static int getTimeWindowIndex(const std::string& pathAndFileName);
  std::map<std::string, int> createInputScopeFileNames(const std::string& inputPathToScopeFiles, std::map<std::string, int> pmPref2ID) const;
