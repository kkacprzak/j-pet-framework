...
private:
  int m_segment_size;
  JPetShapedSignal m_reco_signal;
  FILE* m_file;
};
...
{
  m_reco_signal = ShapedSignalUtils::generateSignal(filename);
...
signal_generation_fixture::signal_generation_fixture() : reco_signal_generator() { reco_signal_generator::setup(gTest_signal_filename); }
...
{
  JPetShapedSignal& sig = reinterpret_cast<JPetShapedSignal&>(m_reader.getCurrentEvent());
...
tref_correctness_fixture::tref_correctness_fixture() : root_file_reader() { root_file_reader::setup(gTest_root_filename); }
