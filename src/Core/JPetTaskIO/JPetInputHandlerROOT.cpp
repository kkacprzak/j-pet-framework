...
JPetInputHandlerROOT::JPetInputHandlerROOT() { fReader = jpet_common_tools::make_unique<JPetReader>(); }

bool JPetInputHandlerROOT::openInput(const char* inputFilename, const JPetParams& params)
{
...
void JPetInputHandlerROOT::closeInput()
{
  if (fReader)
  {
    fReader->closeFile();
...
TObject& JPetInputHandlerROOT::getEntry()
{
  assert(fReader);
...
bool JPetInputHandlerROOT::nextEntry()
{
...
JPetTreeHeader* JPetInputHandlerROOT::getHeaderClone()
{
  assert(fReader);
  return dynamic_cast<JPetReader*>(fReader.get())->getHeaderClone();
}

bool JPetInputHandlerROOT::setEntryRange(const jpet_options_tools::OptsStrAny& options)
{
  bool isOK = false;
  auto firstEntry = 0ll;
  auto lastEntry = 0ll;
...
std::tuple<bool, long long, long long> JPetInputHandlerROOT::calculateEntryRange(const jpet_options_tools::OptsStrAny& options) const
{
