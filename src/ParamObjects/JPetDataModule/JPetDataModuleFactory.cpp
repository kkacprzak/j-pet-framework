...
{
  ParamObjectsDescriptions descriptions = fParamGetter.getAllBasicData(ParamObjectType::kDataModule, fRunID);
  ...
  fInitialized = true;
  ParamRelationalData relations = fParamGetter.getAllRelationalData(ParamObjectType::kDataModule, ParamObjectType::kDataSource, fRunID);
  for (auto relation : relations)
  {
    fDataModules[relation.first]->setDataSource(*fDataSourceFactory.getDataSources().at(relation.second));
  }
}
