...
std::vector<JPetScopeConfigParser::InputDirectory> JPetScopeConfigParser::getInputDirectories(const std::string& configFileLocation,
                                                                                              const scope_config::Config& config) const
{
...
std::vector<JPetScopeConfigParser::FakeInputFile> JPetScopeConfigParser::getFakeInputFileNames(const std::string& configFileName,
                                                                                               const scope_config::Config& config) const
{
...
JPetScopeConfigParser::DirFileContainer JPetScopeConfigParser::getInputDirectoriesAndFakeInputFiles(const std::string& configFileWithPath) const
{
  ...
  else
  {
    ...
    std::transform(directories.begin(), directories.end(), filenames.begin(), std::back_inserter(container),
                   [](InputDirectory dir, FakeInputFile file) { return std::make_pair(dir, file); });
  }
  return getElementsWithExistingDirs(container);
}

JPetScopeConfigParser::DirFileContainer
JPetScopeConfigParser::getElementsWithExistingDirs(const JPetScopeConfigParser::DirFileContainer& dirsAndFiles) const
{
 ...
 */
std::vector<std::string> JPetScopeConfigParser::generateDirectories(const std::string& basePath, const std::vector<int>& positions) const
{
  std::vector<std::string> DirNames(positions.size());
  std::transform(positions.begin(), positions.end(), DirNames.begin(), [&basePath](int number) { return basePath + "/" + std::to_string(number); });
  return DirNames;
}
 ...
 */
std::vector<std::string> JPetScopeConfigParser::generateFileNames(const std::string& configFileName, const std::string& configName,
                                                                  const std::vector<int>& positions) const
{
  ...
  std::transform(positions.begin(), positions.end(), fileNames.begin(),
                 [&configFileName, &configName](int number) { return configFileName + "_" + configName + "_" + std::to_string(number); });
  return fileNames;
}
