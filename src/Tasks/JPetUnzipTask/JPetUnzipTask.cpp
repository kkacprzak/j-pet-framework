...
bool JPetUnzipTask::terminate(JPetParams& outParams)
{
  ...
  auto outputFile = getOutputPath(fOptions) + JPetCommonTools::stripFileNameSuffix(getInputFile(fOptions));
  ...
  INFO(Form("UnzipTask finished, unzipped file name: %s", outputFile.c_str()));
  return true;
}
...
bool JPetUnzipTask::unzipFile(string fileNameWithPath, string outputPath)
{
  ...
  if (JPetCommonTools::exctractFileNameSuffix(fimeName) == ".gz")
  {
    ...
    {
      move = system((string("mv ") + fileNameWithPath.substr(0, fileNameWithPath.size() - 3) + string(" ") + outputPath).c_str());
    }
    else
    {
      move = 0;
  ...
  else if (JPetCommonTools::exctractFileNameSuffix(fimeName) == ".bz2")
  {
    ...
    if (inputPath + string("/") != outputPath && outputPath != "./")
    {
      move = system((string("mv ") + fileNameWithPath.substr(0, fileNameWithPath.size() - 4) + string(" ") + outputPath).c_str());
    }
    else
    {
      move = 0;
  ...
  else if (JPetCommonTools::exctractFileNameSuffix(fimeName) == ".zip")
  {
    ...
    if (inputPath + string("/") != outputPath && outputPath != "./")
    {
      move = system((string("cd ") + JPetCommonTools::currentFullPath() + string(" && ") + string("mv ") + fimeName.substr(0, fimeName.size() - 4) +
                     string(" ") + outputPath)
                        .c_str());
    }
    else
