...
BOOST_AUTO_TEST_CASE(tree_default_constructor)
{
  JPetGeantDecayTree decayTree;
  BOOST_REQUIRE_EQUAL(decayTree.GetEventNumber(), 0);
  BOOST_REQUIRE_EQUAL(decayTree.GetDecayChannel(), DecayChannel::kUnknownDecayChannel);
  BOOST_REQUIRE_EQUAL(decayTree.GetTrackBranchConnections().size(), 0);
}
...
BOOST_AUTO_TEST_CASE(check_decayTree_creation_and_getting_branch)
{
  ...
  auto firstBranch = decayTree.GetDecayTreeBranchByTrack(firstTrackID);
  auto secondBranch = decayTree.GetDecayTreeBranchByTrack(secondTrackID);
  auto thirdBranch = decayTree.GetDecayTreeBranchByTrack(thirdTrackID);
