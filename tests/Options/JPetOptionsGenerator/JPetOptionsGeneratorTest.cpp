...
BOOST_AUTO_TEST_CASE(generateOptions_TwoFilesOneTasks)
{
  JPetOptionsGenerator gener;
  auto inArgs =
      getCmdLineArgs("main.x -i 231 -f unitTestData/JPetOptionsGeneratorTest/infile.root unitTestData/JPetOptionsGeneratorTest/infile2.root -t root");
  auto opt = gener.generateAndValidateOptions(inArgs);
  auto result = gener.generateOptionsForTasks(opt, 1);
  BOOST_REQUIRE(!result.empty());
...
BOOST_AUTO_TEST_CASE(generateOptions_oneFileTwoTasksWithOutput)
{
  JPetOptionsGenerator gener;
  auto inArgs =
      getCmdLineArgs("main.x -i 231 -f unitTestData/JPetOptionsGeneratorTest/infile.root -t root -o unitTestData/JPetCmdParserTest/ -r 2 100");
  auto opt = gener.generateAndValidateOptions(inArgs);
  auto result = gener.generateOptionsForTasks(opt, 2);
  BOOST_REQUIRE(!result.empty());
  BOOST_REQUIRE_EQUAL(result.size(), 1);
...
BOOST_AUTO_TEST_CASE(generateOptions_TestWithUserOptions)
{
  ...
  auto commandLine =
      "main.x -i 231 -f unitTestData/JPetOptionsGeneratorTest/infile.root -t root -u unitTestData/JPetOptionsToolsTest/newInputTestCfg.json";
