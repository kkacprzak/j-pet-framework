...
BOOST_AUTO_TEST_CASE(createInputScopeFileNames)
{
  ...
  BOOST_REQUIRE(
      !reader.createInputScopeFileNames("unitTestData/JPetScopeLoaderTest/scope_files/0", {{"C1", 0}, {"C2", 1}, {"C3", 2}, {"C4", 3}}).empty());
  std::map<std::string, int> expectedRes0{{"C1_00003.txt", 0}, {"C1_00004.txt", 0}, {"C2_00003.txt", 1}, {"C2_00004.txt", 1},
...
BOOST_AUTO_TEST_CASE(generate_root_file)
{
  ...
  auto commandLine =
      "main.exe  -t scope -f unitTestData/JPetScopeLoaderTest/test_file.json -l unitTestData/JPetScopeLoaderTest/test_params.json -i 1";
...
BOOST_AUTO_TEST_CASE(position_does_not_exist)
{
  ...
  boost::filesystem::remove(test_root_filename);
  auto commandLine =
      "main.exe  -t scope -f unitTestData/JPetScopeLoaderTest/wrong_file.json -l unitTestData/JPetScopeLoaderTest/test_params.json -i 1";
  auto args_char = JPetCommonTools::createArgs(commandLine);
  auto argc = args_char.size();
  auto argv = args_char.data();
  JPetManager& manager = JPetManager::getManager();
...
BOOST_AUTO_TEST_CASE(folder_does_not_exist)
{
  ...
  boost::filesystem::remove(test_root_filename);
  auto commandLine =
      "main.exe  -t scope -f unitTestData/JPetScopeLoaderTest/wrong_file2.json -l unitTestData/JPetScopeLoaderTest/test_params.json -i 1";
  auto args_char = JPetCommonTools::createArgs(commandLine);
  auto argc = args_char.size();
  auto argv = args_char.data();
  JPetManager& manager = JPetManager::getManager();
...
BOOST_AUTO_TEST_CASE(generate_root_file2)
{
  ...
  auto commandLine =
      "main.exe  -t scope -f unitTestData/JPetScopeLoaderTest/test_file2.json -l unitTestData/JPetScopeLoaderTest/test_params2.json -i 1";
  auto args_char = JPetCommonTools::createArgs(commandLine);
  auto argc = args_char.size();
  auto argv = args_char.data();
  JPetManager& manager = JPetManager::getManager();
...
{
  std::map<std::string, int> input{{"/some/path/C1_00003.txt", 0}, {"/some/path/C1_00004.txt", 0}, {"/some/path/C2_00003.txt", 1},
                                   {"/some/path/C2_00004.txt", 1}, {"/some/path/C3_00003.txt", 2}, {"/some/path/C3_00004.txt", 2},
                                   {"/some/path/C4_00003.txt", 3}, {"/some/path/C4_00004.txt", 3}};
